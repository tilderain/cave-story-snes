...
# Define the resulting .bnk files for each .it file
AUDIO_BNK_FILES := $(patsubst res/%.it,res/%.bnk,$(AUDIO_IT_FILES))

# NEW: Define the resulting .h files for each .it file (so we can clean them specifically)
AUDIO_HDR_FILES := $(patsubst res/%.it,res/%.h,$(AUDIO_IT_FILES))

SOUNDBANK_ASM_WRAPPER := res/soundbank_vasm.s

# SMCONV Flags:
# -s: silent, -i: HiROM, -V: verbose, -b 3: Bank size
# Note: We do NOT provide -o here because the rule below handles output naming
SMCONVFLAGS = -s -i -V -b 3
...
# 2. Generate the soundbank_vasm.s wrapper dynamically
# This script creates a label for each bank: _soundbank_FILENAME
$(SOUNDBANK_ASM_WRAPPER): $(AUDIO_BNK_FILES)
	@echo "; Automatically generated by Makefile" > $@
	@i=1; for f in $(AUDIO_BNK_FILES); do \
		BASE=`basename $$f .bnk`; \
		echo "    section _rodata.far.audio$$i,\"a\"" >> $@; \
		echo "    align 16" >> $@; \
		echo "    global soundbank_$$BASE" >> $@; \
		echo "soundbank_$$BASE:" >> $@; \
		echo "    incbin \"res/$$BASE.bnk\"" >> $@; \
		i=$$((i+1)); \
	done
...
# Update cleanAudio to remove only generated assets, preserving resources.h
cleanAudio:
	@echo "Cleaning Audio..."
	rm -f $(AUDIO_BNK_FILES) $(AUDIO_HDR_FILES) $(SOUNDBANK_ASM_WRAPPER)